...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await apptsService.createWalkIn({ ...body, requesterId: user.userId })
    ...
    console.error('[POST /api/appointments/walk-in]', err)
    return error('INTERNAL_ERROR', 'Failed to create walk-in', { status: 500 })
  }
}
